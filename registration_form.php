...
$parentFirst = $_SESSION['first_name'] ?? '';
$parentLast  = $_SESSION['last_name']  ?? '';
$parentEmail = $_SESSION['email']      ?? '';
$parentPhone = $_SESSION['phone']      ?? '';
$parentFull  = trim($parentFirst . ' ' . $parentLast);
    ...
    if (!$Sponsor1_Email || !$Sponsor1_Name || !$Sponsor1_Phone_Number || !$Class_Id || !$offering_id) {
        ...
        $stmt = $db->prepare("
            INSERT INTO registrations (
                ...
                Student_Name, Student_Email, Student_Phone_Number, Library_setup, School_support, 
...
<head>
    ...
    <style>
      ...
      input[readonly] { background: #f4f4f4; cursor: not-allowed; }
</style>
</head>
<body>
<?php show_navbar(); ?>
    ...
    <div class="form-content">
        <?php if ($classInfo): ?>
            <div class="header-info">
                ...
                (<?= date('g:i A', strtotime($classInfo['start_time'])) ?> – <?= date('g:i A', strtotime($classInfo['end_time'])) ?>)<br>
                <strong>Dates:</strong>
                <?= date('M j, Y', strtotime($classInfo['start_date'])) ?> – <?= date('M j, Y', strtotime($classInfo['end_date'])) ?><br>
...
<p><br>Registration will be cancelled on <strong>September 22</strong> for non-payment.</p>
 ...
 <h4>How to Pay?</h4>
    <p> Zelle or Venmo: <a href="tel:+16512764671">651-276-4671</a></p>
    <p> If you want to use another method, please reach out to <strong>Siva Jasthi</strong>.</p>
  
 <h4>What Happens Next?</h4>
    <p> Once we receive your registration, we will add the student to our Google Classroom.</p>
    <p>  We also invite you to join  <a href ="https://chat.whatsapp.com/KAbXUzFf4NJEWPUx6Mynqz" target = "_blank"> "Learn and Help" whatsapp group </a> to stay connected. </p>
  
  <p>Thank you for being part of the Learn and Help family! </p>
        ...
        <form method="POST">
            ...
            <label>Parent's (or Sponsor's) Email <span class="required-asterisk">*</span></label>
       ...
       readonly required aria-readonly="true">
            ...
            <label>Parent's (or Sponsor's) Name <span class="required-asterisk">*</span></label>
            <input type="text"  name="Sponsor1_Name"
       value="<?= htmlspecialchars($parentFull, ENT_QUOTES, 'UTF-8') ?>"
       readonly required aria-readonly="true">

          <label>Parent's (or Sponsor's) Phone <span class="required-asterisk">*</span></label>
<input type="text" name="Sponsor1_Phone_Number"
       value="<?= htmlspecialchars($parentPhone, ENT_QUOTES, 'UTF-8') ?>"
       readonly required aria-readonly="true">
       

            <label>Student's Name<span class="required-asterisk">*</span></label>
            <input type="text" name="Student_Name">

            <label>Student's Email<span class="required-asterisk">*</span></label>
